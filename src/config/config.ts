...
export function matchesPattern(values: string[], text: string): boolean { return values.some((pattern) => text.includes(pattern) || new RegExp(pattern).test(text)); }
export function overrideFor(config: HookmarkConfig, text: string): Severity | undefined {
  for (const [pattern, severity] of Object.entries(config.severityOverrides ?? {})) if (text.includes(pattern) || new RegExp(pattern).test(text)) return severity as Severity;
  return undefined;
}
