...
The config root must be a JSON object. `allow` and `ignore` must be arrays of patterns and match against path, trigger, and command text. Empty arrays are valid; non-object roots and non-array values are rejected with a config-path diagnostic. `severityOverrides` can lower or raise a known pattern after human review.
