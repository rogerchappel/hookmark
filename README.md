...
Each value-taking option (`--out`, `--format`, `--fail-on`, and `--config`) may be supplied only once and requires a value that does not begin with `-`. Duplicate options and missing or option-like values exit with code `1` before scanning, reading a config, or writing a report.
